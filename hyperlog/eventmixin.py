...
class EventMixin:
    ...
    async def get_event_colour(
        ...
        defaults = {
            "message_post": discord.Colour.default(),
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if version_info >= VersionInfo.from_str("3.4.0"):
            if await self.bot.cog_disabled_in_guild(self, guild):
                return
        settings = self.settings[guild.id]["message_post"]
        if not settings["enabled"]:
            return
        if message.author.bot and not settings["bots"]:
            return
        try:
            channel = await self.modlog_channel(guild, "message_post")
        except RuntimeError:
            return
        ...
        embed_links = (
                channel.permissions_for(guild.me).embed_links
                and self.settings[guild.id]["message_post"]["embed"]
        )
        if version_info >= VersionInfo.from_str("3.4.1"):
            await i18n.set_contextual_locales_from_guild(self.bot, guild)
        # set guild level i18n
        ...
        if embed_links:
            ...
            embed = discord.Embed(
                ...
                colour=await self.get_event_colour(guild, "message_post"),
            ...
            embed.add_field(name=_("Message:"), value=jump_url)
            embed.add_field(name=_("Channel:"), value=message.channel.mention)
            embed.set_author(
                name=_("{member} ({m_id}) - New Message").format(
                    member=message.author, m_id=message.author.id
                ),
