...
import discord
from redbot.core import Config, checks, commands, modlog, errors
...
_ = Translator("HyperLog", __file__)
logger = logging.getLogger("red.rokuro-cogs.HyperLog")


@cog_i18n(_)
class HyperLog(EventMixin, commands.Cog):
    """
    Extended modlogs
    Works with core modlogset channel
    """

    __author__ = ["bakaneko","RePulsar", "TrustyJAID"]
    __version__ = "1.0.1"
    ...
    def __init__(self, bot):
        ...
        #16桁で作成
        self.config = Config.get_conf(self, 1732510834241877, force_registration=True)#todo
    ...
    @commands.group(name="hyperlog", aliases=["hyperlogtoggle", "hyperlogs"])
    @commands.guild_only()
    async def _hyperlog(self, ctx: commands.Context) -> None:
        """
        Toggle various extended hyperlog notifications

        Requires the channel to be setup with `[p]hyperlogset hyperlog #channel`
        Or can be sent to separate channels with `[p]hyperlog channel #channel event_name`
        """
        pass

    @_hyperlog.command(name="settings")
    async def _show_modlog_settings(self, ctx: commands.Context):
        """
        Show the servers current hyperlog settings
        """
        ...
        await self.hyperlog_settings(ctx)

    @_hyperlog.command(name="colour", aliases=["color"])
    ...
    @_hyperlog.command(name="embeds", aliases=["embed"])
    ...
    @_hyperlog.command(name="emojiset", send_help=True)
    ...
    @_hyperlog.command(name="channel")
    ...
    @_hyperlog.command(name="resetchannel")
    ...
    @_hyperlog.command(name="all", aliaes=["all_settings", "toggle_all"])
    ...
    @_hyperlog.command(name="botedits", aliases=["botedit"])
    ...
    @_hyperlog.command(name="botdeletes", aliases=["botdelete"])
    ...
    @_hyperlog.command(name="botchange")
    ...
    @_hyperlog.command(name="nickname", aliases=["nicknames"])
    ...
    @_hyperlog.command(name="commandlevel", aliases=["commandslevel"])
